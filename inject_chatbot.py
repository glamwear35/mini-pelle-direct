#!/usr/bin/env python3
"""
Mini Pelle Direct — Injection automatique du chatbot
Crée chatbot-mpd.js et l'injecte dans tous les fichiers HTML du projet.
Usage : python3 inject_chatbot.py
"""
...
PROJECT_DIR = Path.home() / "Desktop" / "mini-pelle-direct"
# Si ton projet est ailleurs, change cette ligne :
# PROJECT_DIR = Path("/chemin/vers/ton/projet")

# ─── CONTENU DU CHATBOT JS ───────────────────────────────────────────────────
CHATBOT_JS = r"""
/* ============================================================
  ...
  async function sendNotif(name, phone, email, ctx) {
    try {
      if (typeof emailjs !== 'undefined') {
        await emailjs.send('service_minipelle', 'template_mpd_prospect', {
          to_email: 'minipelledirect@gmail.com',
          subject: `🚜 Nouveau prospect — ${name}`,
          message: `Nom : ${name}\nTél : ${phone}\nEmail : ${email||'—'}\n\nDétails :\n${ctx}\n\n⏰ ${new Date().toLocaleString('fr-FR')}`,
          reply_to: email || 'minipelledirect@gmail.com'
        });
      }
  ...
  async function handleIntent(intent) {
    switch(intent) {
      ...
      case 'dur_journee': case 'dur_weekend': case 'dur_semaine': {
        ...
        await botReply(`Voilà votre estimation 👆<br><br>Livraison incluse dans un rayon de 30 km autour de Bain-de-Bretagne.<br>Pour confirmer, laissez vos coordonnées 👇`, 1000);
        showContactForm(`Location ${devisData.machine==='kpc'?'KPC KT562 800kg':'Kubota D902 1T5'} — ${duree} — ~${total}€`);
        break;
      }
      case 'aide_choix':
...
# ─── BALISE À INJECTER ───────────────────────────────────────────────────────
INJECT_TAG = '<script src="/chatbot-mpd.js"></script>'
INJECT_MARKER = "<!-- chatbot-mpd -->"

# ─── PAGES À EXCLURE ─────────────────────────────────────────────────────────
EXCLUDE_FILES = {"admin.html", "404.html"}

def backup_project(project_dir):
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = project_dir.parent / f"mini-pelle-direct_backup_{ts}"
    shutil.copytree(project_dir, backup)
    print(f"✅ Backup créé : {backup}")

def create_chatbot_js(project_dir):
    js_path = project_dir / "chatbot-mpd.js"
    js_path.write_text(CHATBOT_JS.strip(), encoding="utf-8")
    print(f"✅ Fichier créé : {js_path}")

def inject_into_html(html_path):
    content = html_path.read_text(encoding="utf-8")

    # Déjà injecté ?
    if INJECT_MARKER in content or "chatbot-mpd.js" in content:
        print(f"  ⏭️  Déjà injecté : {html_path.name}")
        return False
    ...
    if "</body>" in content:
        ...
        print(f"  ⚠️  Pas de </body> trouvé dans {html_path.name}, ajout en fin de fichier")

    html_path.write_text(new_content, encoding="utf-8")
    print(f"  ✅ Injecté : {html_path.name}")
    return True

def main():
    print("\n🚜 Mini Pelle Direct — Injection chatbot\n" + "="*45)

    if not PROJECT_DIR.exists():
        print(f"❌ Projet introuvable : {PROJECT_DIR}")
        print("   Modifie la variable PROJECT_DIR dans ce script.")
        return
    ...
    # 2. Créer chatbot-mpd.js
    create_chatbot_js(PROJECT_DIR)

    # 3. Injecter dans tous les HTML
    ...
    for f in html_files:
        if f.name in EXCLUDE_FILES:
            ...
            skipped += 1
            continue
        if inject_into_html(f):
            injected += 1
        else:
            skipped += 1

    print(f"\n{'='*45}")
    print(f"✅ {injected} fichier(s) modifié(s)")
    print(f"⏭️  {skipped} fichier(s) ignoré(s)")
